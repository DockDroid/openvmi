...
#include "openvmi/network/base_socket_messenger.h"
...
void BaseSocketMessenger<stream_protocol>::setup(
  ...
  socket = s;
  socket_fd = openvmi::Fd{IntOwnedFd{socket->native_handle()}};
...
void BaseSocketMessenger<stream_protocol>::async_receive_msg(
    OpenvmiReadHandler const& handler, ba::mutable_buffers_1 const& buffer) {
