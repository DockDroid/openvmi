...
#include "openvmi/graphics/emugl/DispatchTables.h"
#include "openvmi/graphics/emugl/DisplayManager.h"
#include "openvmi/graphics/emugl/RenderThreadInfo.h"
#include "openvmi/graphics/emugl/Renderer.h"
#include "openvmi/graphics/emugl/RendererConfig.h"
...
static std::shared_ptr<openvmi::graphics::LayerComposer> composer;
static std::shared_ptr<Renderer> renderer;

void registerLayerComposer(
    const std::shared_ptr<openvmi::graphics::LayerComposer> &c) {
  composer = c;
}
...
static std::string filter_extensions(const std::string& extensions, const std::vector<std::string>& whitelist) {
  ...
  auto extension_list = openvmi::utils::string_split(extensions, ' ');
...
static EGLint rcGetGLString(EGLenum name, void* buffer, EGLint bufferSize) {
  ...
  if (name == GL_EXTENSIONS) {
      ...
      result += "GL_OES_vertex_array_object ";
  }
...
static EGLint rcGetFBParam(EGLint param) {
  ...
  switch (param) {
    case FB_WIDTH:
      ret = static_cast<EGLint>(openvmi::graphics::emugl::DisplayInfo::get()->vertical_resolution());
      break;
    case FB_HEIGHT:
      ret = static_cast<EGLint>(openvmi::graphics::emugl::DisplayInfo::get()->horizontal_resolution());
      break;
    case FB_XDPI:
...
static void rcReadColorBuffer(uint32_t colorBuffer, GLint x, GLint y,
  ...
  if(++count ==100){
      long long timer1 = 1000000 * (start.tv_sec-lastReadTime.tv_sec)+ start.tv_usec-lastReadTime.tv_usec;
      DEBUG("timer between rcReadColorBuffer=%d\n, read_fps=%.1f", timer1, 100000000.0/timer1);
      lastReadTime = start;
      count = 0;
  }
  ...
  long long timer2 = 1000000 * (end.tv_sec-start.tv_sec)+ end.tv_usec-start.tv_usec;
  DEBUG("delay for rcReadColorBuffer=%d\n", timer2);
}
...
int rcGetDisplayWidth(uint32_t display_id) {
  (void)display_id;
  return static_cast<int>(openvmi::graphics::emugl::DisplayInfo::get()->vertical_resolution());
}

int rcGetDisplayHeight(uint32_t display_id) {
  (void)display_id;
  return static_cast<int>(openvmi::graphics::emugl::DisplayInfo::get()->horizontal_resolution());
}
...
void rcPostAllLayersDone() {
  ...
  if(++count ==100){
      long long timer1 = 1000000 * (start.tv_sec-lastReadTime.tv_sec)+ start.tv_usec-lastReadTime.tv_usec;
      DEBUG("timer between rcPostAllLayersDone=%d\n, render_fps=%.1f", timer1, 100000000.0/timer1);
      lastReadTime = start;
      count = 0;
  }
