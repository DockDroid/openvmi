...
#include "openvmi/graphics/emugl/TextureResize.h"
...
void TextureResize::setupFramebuffers(unsigned int factor) {
  ...
  s_gles2.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, mWidth / factor, mHeight, 0,
                       GL_RGB, mTextureDataType, nullptr);
  ...
  s_gles2.glBindTexture(GL_TEXTURE_2D, mFBHeight.texture);
  s_gles2.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, mWidth / factor, mHeight / factor, 0,
                       GL_RGB, mTextureDataType, nullptr);
  s_gles2.glBindTexture(GL_TEXTURE_2D, 0);
