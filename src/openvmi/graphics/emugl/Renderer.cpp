...
#include "openvmi/graphics/emugl/TimeUtils.h"
#include "openvmi/graphics/gl_extensions.h"
...
bool Renderer::initialize(EGLNativeDisplayType nativeDisplay) {
  ...
  openvmi::graphics::GLExtensions egl_extensions{s_egl.eglQueryString(m_eglDisplay, EGL_EXTENSIONS)};
  ...
  openvmi::graphics::GLExtensions gl_extensions{reinterpret_cast<const char *>(s_gles2.glGetString(GL_EXTENSIONS))};
...
struct RendererWindow {
  ...
  openvmi::graphics::Rect viewport;
...
void Renderer::setupViewport(RendererWindow *window,
                             const openvmi::graphics::Rect &rect) {
  /*
...
void Renderer::tessellate(std::vector<openvmi::graphics::Primitive> &primitives,
                          const openvmi::graphics::Rect &buf_size,
  ...
  openvmi::graphics::Primitive rectangle;
  rectangle.tex_id = 0;
...
void Renderer::draw(RendererWindow *window, const Renderable &renderable,
  ...
  for (auto const &p : m_primitives) {
    ...
    s_gles2.glVertexAttribPointer(prog.position_attr, 3, GL_FLOAT, GL_FALSE,
                                  sizeof(openvmi::graphics::Vertex),
...
bool Renderer::draw(EGLNativeWindowType native_window,
                    const openvmi::graphics::Rect &window_frame,
...
void Renderer::getScreenPixels(EGLNativeWindowType native_window,int width, int height, void* pixels){
	std::unique_lock<std::mutex> l(m_lock);
	  auto w = m_nativeWindows.find(native_window);
	  if (w == m_nativeWindows.end()) return ;
...
void Renderer::getScreenPixelsEncturbo(EGLNativeWindowType native_window,int width, int height, void* pixels){
	std::unique_lock<std::mutex> l(m_lock);
	  auto w = m_nativeWindows.find(native_window);
	  if (w == m_nativeWindows.end()) return ;
...
//	  s_gles2.glnativeTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0,
//             GL_RGBA, GL_UNSIGNED_BYTE, &pixels, NULL);
//          s_gles2.glReadPixels(0, 0, width, height,
//	    		GL_RGBA , GL_UNSIGNED_BYTE, pixels);
	  unbind_locked();
}
