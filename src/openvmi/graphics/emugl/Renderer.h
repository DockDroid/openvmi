...
#include "openvmi/graphics/emugl/ColorBuffer.h"
#include "openvmi/graphics/emugl/RenderContext.h"
#include "openvmi/graphics/emugl/RendererConfig.h"
#include "openvmi/graphics/emugl/TextureDraw.h"
#include "openvmi/graphics/emugl/WindowSurface.h"
#include "openvmi/graphics/emugl/Renderable.h"

#include "openvmi/graphics/primitives.h"
#include "openvmi/graphics/program_family.h"
#include "openvmi/graphics/renderer.h"
...
//
class Renderer : public openvmi::graphics::Renderer {
 public:
  Renderer();
  virtual ~Renderer();
  ...
  void getScreenPixels(EGLNativeWindowType native_window, int width, int height, void* pixels);
  void getScreenPixelsEncturbo(EGLNativeWindowType native_window, int width, int height, void* pixels);
 ...
 private:
  ...
  void setupViewport(RendererWindow* window, const openvmi::graphics::Rect& rect);
  struct Program;
  void draw(RendererWindow* window, const Renderable& renderable,
            const Program& prog);
  void tessellate(std::vector<openvmi::graphics::Primitive>& primitives,
 ...
 private:
  ...
  openvmi::graphics::ProgramFamily m_family;
  struct Program {
    GLuint id = 0;
    GLint tex_uniform = -1;
  ...
  std::vector<openvmi::graphics::Primitive> m_primitives;
