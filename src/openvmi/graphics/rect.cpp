...
std::istream& operator>>(std::istream& in, openvmi::graphics::Rect &rect)
try {
  std::string str;
  in >> str;
  auto tokens = openvmi::utils::string_split(str, ',');

  switch (tokens.size()) {
  case 2: {
    rect = openvmi::graphics::Rect(0, 0,
             boost::lexical_cast<std::int32_t>(tokens[0]),
             boost::lexical_cast<std::int32_t>(tokens[1]));
    break;
  }
  case 4:
    rect = openvmi::graphics::Rect(
             boost::lexical_cast<std::int32_t>(tokens[0]),
             boost::lexical_cast<std::int32_t>(tokens[1]),
