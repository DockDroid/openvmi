...
#ifndef OPENVMI_GRAPHICS_BUFFERED_IO_STREAM_H_
#define OPENVMI_GRAPHICS_BUFFERED_IO_STREAM_H_

#include "external/android-emugl/host/include/libOpenglRender/IOStream.h"

#include "openvmi/graphics/buffer_queue.h"
...
class BufferedIOStream : public IOStream {
 public:
  ...
  explicit BufferedIOStream(
      const std::shared_ptr<openvmi::network::SocketMessenger> &messenger,
 ...
 private:
  ...
  std::shared_ptr<openvmi::network::SocketMessenger> messenger_;
  std::mutex lock_;
  std::mutex out_lock_;
