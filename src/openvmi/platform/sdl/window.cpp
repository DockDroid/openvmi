...
#include "openvmi/graphics/density.h"
...
Window::Window(const std::shared_ptr<Renderer> &renderer,
...
#if 0 //solve click on top right-hand corner close window bug. add by zhengzhibing.
  if (utils::get_env_value("OPENVMI_NO_SDL_WINDOW_HIT_TEST", "false") == "false")
  ...
  INFO("info.subsystem=%d", info.subsystem);
