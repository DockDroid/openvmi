...
#include "openvmi/utils/environment_file.h"
...
class SystemInformation {
 public:
  ...
  std::string to_text() const {
    ...
    s << "version: "
      << openvmi::build::version
      << std::endl;

    if (openvmi::utils::is_env_set("SNAP_REVISION")) {
      s << "snap-revision: "
        << openvmi::utils::get_env_value("SNAP_REVISION")
        << std::endl;
    }
 ...
 private:
  ...
  void collect_os_info() {
    os_info_.snap_based = !openvmi::utils::get_env_value("SNAP").empty();
    ...
    if (fs::exists(path)) {
      openvmi::utils::EnvironmentFile os_release(path);
  ...
  void collect_graphics_info() {
    auto gl_libs = openvmi::graphics::emugl::default_gl_libraries();
    if (!openvmi::graphics::emugl::initialize(gl_libs, nullptr, nullptr)) {
      return;
    }
    ...
    if (display != EGL_NO_DISPLAY) {
      ...
      graphics_info_.egl_extensions = openvmi::utils::string_split(egl_extensions, ' ');
      ...
      if (s_egl.eglChooseConfig(display, config_attribs, &config, 1, &n) && n > 0) {
        ...
        if (context != EGL_NO_CONTEXT) {
          ...
          graphics_info_.gles2_extensions = openvmi::utils::string_split(gl_extensions, ' ');
...
openvmi::cmds::SystemInfo::SystemInfo()
