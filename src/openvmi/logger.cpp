...
namespace {
namespace attrs {
BOOST_LOG_ATTRIBUTE_KEYWORD(Severity, "Severity", openvmi::Logger::Severity)
BOOST_LOG_ATTRIBUTE_KEYWORD(Location, "Location", openvmi::Logger::Location)
BOOST_LOG_ATTRIBUTE_KEYWORD(Timestamp, "Timestamp", boost::posix_time::ptime)
}

struct BoostLogLogger : public openvmi::Logger {
  BoostLogLogger() : initialized_(false) {}

  void Init(const openvmi::Logger::Severity& severity = openvmi::Logger::Severity::kWarning) override {
    if (initialized_)
      return;
  ...
  void Log(Severity severity, const std::string& message, const boost::optional<Location>& loc) override {
    ...
    if (auto rec = boost::log::trivial::logger::get().open_record()) {
      ...
      out << boost::log::add_value(attrs::Severity, severity)
          << boost::log::add_value(attrs::Timestamp, boost::posix_time::second_clock::local_time())
          << boost::log::add_value(attrs::Location, tmp) 
          << "[" << unique_Name << "] "
          << "[" << unique_ID << "] "
          << severity
          << "[" << loc << "] "
...
std::shared_ptr<openvmi::Logger>& MutableInstance() {
  static std::shared_ptr<openvmi::Logger> instance{new BoostLogLogger()};
  return instance;
}

void SetInstance(const std::shared_ptr<openvmi::Logger>& logger) {
...
std::ostream& operator<<(std::ostream& strm, openvmi::Logger::Severity severity) {
  switch (severity) {
    case openvmi::Logger::Severity::kTrace:
      return strm << "[trace] ";
    case openvmi::Logger::Severity::kDebug:
      return strm << "[debug] ";
    case openvmi::Logger::Severity::kInfo:
      return strm << "[info] ";
    case openvmi::Logger::Severity::kWarning:
      return strm << "[warning] ";
    case openvmi::Logger::Severity::kError:
      return strm << "[error] ";
    case openvmi::Logger::Severity::kFatal:
      return strm << "[fatal] ";
    default:
