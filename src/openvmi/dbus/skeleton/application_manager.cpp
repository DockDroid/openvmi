...
#include "openvmi/dbus/skeleton/application_manager.h"
#include "openvmi/dbus/interface.h"
#include "openvmi/dbus/sd_bus_helpers.hpp"
#include "openvmi/android/intent.h"
...
int ApplicationManager::method_launch(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
  ...
  if (intent.package.length() == 0) {
    sd_bus_error_set_const(ret_error, "org.openvmi.InvalidArgument", "No package specified");
    return -EINVAL;
  }
  ...
  try {
    ...
    sd_bus_error_set_const(ret_error, "org.openvmi.InternalError", err.what());
    return -EIO;
  }
...
ApplicationManager::ApplicationManager(const BusPtr& bus, const std::shared_ptr<openvmi::application::Manager> &impl)
    : bus_(bus), impl_(impl) {
...
void ApplicationManager::launch(const android::Intent &intent,
  ...
  if (!impl_->ready())
    throw std::runtime_error("Openvmi not yet ready to launch applications");
