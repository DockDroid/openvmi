...
namespace dbus {
namespace skeleton {
std::shared_ptr<Service> Service::create_for_bus(const BusPtr& bus, const std::shared_ptr<openvmi::application::Manager> &impl) {
  return std::shared_ptr<Service>(new Service(bus, impl));
}

Service::Service(const BusPtr& bus, const std::shared_ptr<openvmi::application::Manager> &impl)
    : bus_{bus} {
  if (!bus_)
