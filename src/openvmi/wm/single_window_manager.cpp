...
void SingleWindowManager::setup() {
  if (auto p = platform_.lock()) {
    window_ = p->create_window(0, window_size_, _G_stBuf);
    if (!window_->attach())
...
void SingleWindowManager::resize_task(const Task::Id &task, const openvmi::graphics::Rect &rect,
