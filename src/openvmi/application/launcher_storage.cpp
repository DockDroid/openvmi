...
// a snap environment.
constexpr const char *snap_exe_path{"/snap/bin/openvmi"};
}
  ...
  if (fs::exists(path_)) {
    for(auto & p : boost::filesystem::directory_iterator(path_)) {
        if (fs::is_regular_file(p)){
          ...
          if (boost::starts_with(str, "openvmi-"))
            fs::remove(p);
        }
     }
   }
...
fs::path LauncherStorage::path_for_item(const std::string &package_name) {
  return path_ / utils::string_format("openvmi-%s.desktop", package_name);
}

fs::path LauncherStorage::path_for_item_icon(const std::string &package_name) {
  return path_ / utils::string_format("openvmi-%s.png", package_name);
}
