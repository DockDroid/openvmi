...
#define DEFINE_AND_VALIDATE_HOST_CONNECTION() \
    HostConnection *hostCon = HostConnection::get(); \
    if (!hostCon) { \
        ALOGE("hwcomposer.openvmi: Failed to get host connection\n"); \
        return -EIO; \
    } \
    renderControl_encoder_context_t *rcEnc = hostCon->rcEncoder(); \
    if (!rcEnc) { \
        ALOGE("hwcomposer.openvmi: Failed to get renderControl encoder context\n"); \
        return -EIO; \
    }
...
static int hwc_prepare(hwc_composer_device_1_t* dev, size_t numDisplays,
    ...
    // Openvmi only supports the primary display.
