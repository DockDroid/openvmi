...
void GLClientState::setState(int location, int size, GLenum type,
        GLboolean normalized, GLsizei stride, const void *data) {
    if (!validLocation(location)) {
        return;
    }
...
const GLClientState::VertexAttribState* GLClientState::getStateAndEnableDirty(
...
size_t GLClientState::pixelDataSize(GLsizei width, GLsizei height,
        GLenum format, GLenum type, int pack) const {
    if (width <= 0 || height <= 0)
        return 0;
    ...
    for (const GLuint *texture = textures; texture != textures + n; texture++) {
        ...
        if (texrec) {
            ...
            for (TextureUnit *unit = m_tex.unit;
                    unit != m_tex.unit + MAX_TEXTURE_UNITS; unit++) {
