...
void glUtilsPackPointerData(unsigned char *dst, unsigned char *src, int size,
        GLenum type, unsigned int stride, unsigned int datalen) {
...
void glUtilsWritePackPointerData(void *_stream, unsigned char *src, int size,
        GLenum type, unsigned int stride, unsigned int datalen) {
...
int glUtilsPixelBitSize(GLenum format, GLenum type) {
    ...
    default:
        ERR(
                "glUtilsPixelBitSize: unknown pixel type - assuming pixel data 0\n");
        componentsize = 0;
    }
...
void glUtilsPackStrings(char *ptr, char **strings, GLint *length,
        GLsizei count) {
    char *p = ptr;
    *p = '\0';
