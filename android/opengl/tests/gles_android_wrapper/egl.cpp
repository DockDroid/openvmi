...
template<class T>
int initApi(const char *driverLibName, const char *implLibName,
        T **dispatchTable, T* (*accessor)()) {
...
void initDispatchTables() {
    ...
    void *gles_android = dlopen("/system/lib/egl/libGLES_android.so",
            RTLD_NOW | RTLD_LOCAL);
    if (!gles_android) {
    ...
    if (s_needEncode) {
        ...
        ServerConnection *connection =
                ServerConnection::s_getServerConnection();
        if (connection == NULL) {
    ...
    if (s_needEncode) {
        ...
        if (initApi<gl_wrapper_context_t>(GLESv1_DRIVER, GLESv1_enc_LIB,
                &g_gl_dispatch, getGLContext) < 0) {
            // fallback to android on faluire
            s_needEncode = false;
        } else {
            initApi<gl2_wrapper_context_t>(GLESv2_DRIVER, GLESv2_enc_LIB,
                    &g_gl2_dispatch, getGL2Context);
        }
    }

    if (!s_needEncode) {
        ALOGD("Initializing native opengl for %s\n", getProcName());
        initApi<gl_wrapper_context_t>(GLESv1_DRIVER, GLES_android_LIB,
                &g_gl_dispatch, getGLContext);
        // try to initialize gl2 from GLES, though its probably going to fail
        initApi<gl2_wrapper_context_t>(GLESv2_DRIVER, GLES_android_LIB,
                &g_gl2_dispatch, getGL2Context);
    }
}
...
EGLBoolean eglChooseConfig(EGLDisplay dpy, const EGLint *attrib_list,
    ...
    if (s_needEncode) {
        ...
        res = getDispatch()->eglChooseConfig(dpy, attribs, configs, config_size,
                num_config);
        ...
        res = getDispatch()->eglChooseConfig(dpy, attrib_list, configs,
...
EGLSurface eglCreateWindowSurface(EGLDisplay dpy, EGLConfig config,
    ...
    EGLSurface surface = getDispatch()->eglCreateWindowSurface(dpy, config, win,
            attrib_list);
    if (surface != EGL_NO_SURFACE) {
        ServerConnection *server;
        if (s_needEncode
                && (server = ServerConnection::s_getServerConnection())
                        != NULL) {
            server->utEnc()->createSurface(server->utEnc(), getpid(),
                    (uint32_t) surface);
        }
    }
    return surface;
...
EGLSurface eglCreatePbufferSurface(EGLDisplay dpy, EGLConfig config,
    ...
    EGLSurface surface = getDispatch()->eglCreatePbufferSurface(dpy, config,
            attrib_list);
    if (surface != EGL_NO_SURFACE) {
        ServerConnection *server;
        if (s_needEncode
...
EGLSurface eglCreatePixmapSurface(EGLDisplay dpy, EGLConfig config,
    ...
    EGLSurface surface = getDispatch()->eglCreatePixmapSurface(dpy, config,
            pixmap, attrib_list);
    if (surface != EGL_NO_SURFACE) {
        ServerConnection *server;
        if (s_needEncode
...
EGLBoolean eglDestroySurface(EGLDisplay dpy, EGLSurface surface) {
    ...
    if (res && surface != EGL_NO_SURFACE) {
        ...
        if (s_needEncode
                && (server = ServerConnection::s_getServerConnection())
                        != NULL) {
            server->utEnc()->destroySurface(server->utEnc(), getpid(),
                    (uint32_t) surface);
        }
    }
    return res;
...
EGLBoolean eglQuerySurface(EGLDisplay dpy, EGLSurface surface, EGLint attribute,
        EGLint *value) {
    EGLBoolean res = getDispatch()->eglQuerySurface(dpy, surface, attribute,
            value);
...
EGLSurface eglCreatePbufferFromClientBuffer(EGLDisplay dpy, EGLenum buftype,
    ...
    return getDispatch()->eglCreatePbufferFromClientBuffer(dpy, buftype, buffer,
            config, attrib_list);
}
...
EGLContext eglCreateContext(EGLDisplay dpy, EGLConfig config,
    ...
    if (share)
        share = ((EGLWrapperContext*) share_context)->aglContext;
    ...
    int version = 1;
    for (int i = 0; i < nAttribs; i++) {
        if (attrib[i] == EGL_CONTEXT_CLIENT_VERSION && attrib[i + 1] == 2) {
            version = 2;
            attrib[i + 1] = 1; // replace to version 1
        }
    }

    EGLContext ctx = getDispatch()->eglCreateContext(dpy, config, share,
            attrib);
    delete attrib;
...
EGLBoolean eglDestroyContext(EGLDisplay dpy, EGLContext ctx) {
    ...
    if (ctx && ctx != EGL_NO_CONTEXT) {
        ...
        if (res) {
            ...
            ServerConnection *server;
            if (s_needEncode && (server =
                    ServerConnection::s_getServerConnection())) {
                server->utEnc()->destroyContext(ti->serverConn->utEnc(),
                        getpid(), (uint32_t) ctx);
            }
            if (ti->currentContext == wctx)
                ti->currentContext = NULL;
            delete wctx;
        }
    }
...
EGLBoolean eglMakeCurrent(EGLDisplay dpy, EGLSurface draw, EGLSurface read,
    ...
    EGLContext aglContext = (
            ctx == EGL_NO_CONTEXT ? EGL_NO_CONTEXT : wctx->aglContext);
    EGLThreadInfo *ti = getEGLThreadInfo();
    ...
    if (res) {
        ...
        if (s_needEncode
            ...
            server->glEncoder()->setClientState(
                    wctx ? wctx->clientState : NULL);
            server->gl2Encoder()->setClientState(
                    wctx ? wctx->clientState : NULL);
        }
    ...
    if (wctx) {
        if (attribute == EGL_CONTEXT_CLIENT_VERSION) {
            *value = wctx->version;
            ...
            return getDispatch()->eglQueryContext(dpy, wctx->aglContext,
                    attribute, value);
        }
    } else {
...
EGLBoolean eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ServerConnection *server;
    if (s_needEncode
            && (server = ServerConnection::s_getServerConnection()) != NULL) {
        server->utEnc()->swapBuffers(server->utEnc(), getpid(),
                (uint32_t) surface);
...
EGLBoolean eglLockSurfaceKHR(EGLDisplay display, EGLSurface surface,
        const EGLint *attrib_list) {
...
EGLImageKHR eglCreateImageKHR(EGLDisplay dpy, EGLContext ctx, EGLenum target,
    ...
    return getDispatch()->eglCreateImageKHR(dpy, aglContext, target, buffer,
            attrib_list);
}
...
EGLBoolean eglSetSwapRectangleANDROID(EGLDisplay dpy, EGLSurface draw,
    ...
    return getDispatch()->eglSetSwapRectangleANDROID(dpy, draw, left, top,
            width, height);
}
