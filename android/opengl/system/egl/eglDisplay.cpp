...
eglDisplay::eglDisplay() :
        m_initialized(false), m_major(0), m_minor(0), m_hostRendererVersion(0), m_numConfigs(
                0), m_numConfigAttribs(0), m_attribs(
                DefaultKeyedVector<EGLint, EGLint>(ATTRIBUTE_NONE)), m_configs(
                NULL), m_gles_iface(NULL), m_gles2_iface(NULL), m_versionString(
                NULL), m_vendorString(NULL), m_extensionString(NULL) {
...
bool eglDisplay::initialize(EGLClient_eglInterface *eglIface) {
    ...
    if (!m_initialized) {
        ...
        m_gles_iface = loadGLESClientAPI("/system/lib64/egl/libGLESv1_CM_emulation.so",
                                         ...
                                         &s_gles_lib);
#else
        m_gles_iface = loadGLESClientAPI(
                "/system/lib/egl/libGLESv1_CM_emulation.so", eglIface,
                &s_gles_lib);
#endif
        if (!m_gles_iface) {
        ...
        //
        m_numConfigs = rcEnc->rcGetNumConfigs(rcEnc,
                (uint32_t*) &m_numConfigAttribs);
        ...
        EGLint n = rcEnc->rcGetConfigs(rcEnc, nInts * sizeof(EGLint),
                (GLuint*) tmp_buf);
        if (n != m_numConfigs) {
            pthread_mutex_unlock(&m_lock);
            return false;
...
EGLClient_glesInterface* eglDisplay::loadGLESClientAPI(const char *libName,
        EGLClient_eglInterface *eglIface, void **libHandle) {
    ...
    init_emul_gles_t init_gles_func = (init_emul_gles_t) dlsym(lib,
            "init_emul_gles");
    if (!init_gles_func) {
...
const char* eglDisplay::queryString(EGLint name) {
    if (name == EGL_CLIENT_APIS) {
        ...
        if (hostVendor) {
            asprintf(&m_vendorString, "%s Host: %s", systemEGLVendor,
                    hostVendor);
 ...
 */
EGLBoolean eglDisplay::getAttribValue(EGLConfig config, EGLint attribIdx,
        EGLint *value) {
...
EGLBoolean eglDisplay::getConfigAttrib(EGLConfig config, EGLint attrib,
        EGLint *value) {
 ...
 */
EGLBoolean eglDisplay::setAttribValue(EGLConfig config, EGLint attribIdx,
        EGLint value) {
    if (attribIdx == ATTRIBUTE_NONE) {
        ALOGE("[%s] Bad attribute idx\n", __FUNCTION__);
        return EGL_FALSE;
...
EGLBoolean eglDisplay::setConfigAttrib(EGLConfig config, EGLint attrib,
        EGLint value) {
    //Though it seems that valueFor() is thread-safe, we don't take chanses
    pthread_mutex_lock(&m_lock);
...
EGLBoolean eglDisplay::getConfigNativePixelFormat(EGLConfig config,
        PixelFormat *format) {
    EGLint redSize, blueSize, greenSize, alphaSize;

    if (!(getAttribValue(config, m_attribs.valueFor(EGL_RED_SIZE), &redSize)
            && getAttribValue(config, m_attribs.valueFor(EGL_BLUE_SIZE),
                    &blueSize)
            && getAttribValue(config, m_attribs.valueFor(EGL_GREEN_SIZE),
                    &greenSize)
            && getAttribValue(config, m_attribs.valueFor(EGL_ALPHA_SIZE),
                    &alphaSize))) {
    ...
    if ((redSize == 8) && (greenSize == 8) && (blueSize == 8)
            && (alphaSize == 8))
        *format = PIXEL_FORMAT_RGBA_8888; //XXX: BGR?
    else if ((redSize == 8) && (greenSize == 8) && (blueSize == 8)
            && (alphaSize == 0))
        *format = PIXEL_FORMAT_RGBX_8888; //XXX or PIXEL_FORMAT_RGB_888
    else if ((redSize == 5) && (greenSize == 6) && (blueSize == 5)
            && (alphaSize == 0))
        *format = PIXEL_FORMAT_RGB_565;
    else if ((redSize == 5) && (greenSize == 5) && (blueSize == 5)
            && (alphaSize == 1))
        *format = PIXEL_FORMAT_RGBA_5551;
    else if ((redSize == 4) && (greenSize == 4) && (blueSize == 4)
            && (alphaSize == 4))
...
EGLBoolean eglDisplay::getConfigGLPixelFormat(EGLConfig config,
        GLenum *format) {
    EGLint redSize, blueSize, greenSize, alphaSize;
    ...
    if ((redSize == greenSize) && (redSize == blueSize)
        ...
        if (alphaSize == 0)
            ...
            *format = GL_RGBA;
            ...
            && (alphaSize == 0))
        *format = GL_RGB565_OES;
    else if ((redSize == 5) && (greenSize == 5) && (blueSize == 5)
            && (alphaSize == 1))
        *format = GL_RGB5_A1_OES;
    else if ((redSize == 4) && (greenSize == 4) && (blueSize == 4)
            && (alphaSize == 4))
        *format = GL_RGBA4_OES;
    else
