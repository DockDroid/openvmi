...
EGLContext_t::EGLContext_t(EGLDisplay dpy, EGLConfig config,
        ...
        dpy(dpy), config(config), read(EGL_NO_SURFACE), draw(EGL_NO_SURFACE), shareCtx(
                shareCtx), rcContext(0), versionString(NULL), vendorString(
                NULL), rendererString(NULL), shaderVersionString(NULL), extensionString(
                NULL), deletePending(0) {
    flags = 0;
    version = 1;
...
struct egl_surface_t {
    ...
    void setTextureFormat(EGLint _texFormat) {
        texFormat = _texFormat;
    }
    EGLint getTextureFormat() {
        return texFormat;
    }
    void setTextureTarget(EGLint _texTarget) {
        texTarget = _texTarget;
    }
    EGLint getTextureTarget() {
        return texTarget;
    }
...
protected:
    void setWidth(EGLint w) {
        width = w;
    }
    void setHeight(EGLint h) {
        height = h;
    }
...
struct egl_window_surface_t: public egl_surface_t {
    static egl_window_surface_t* create(EGLDisplay dpy, EGLConfig config,
            EGLint surfType, ANativeWindow *window);
...
private:
    egl_window_surface_t(EGLDisplay dpy, EGLConfig config, EGLint surfType,
...
egl_window_surface_t::egl_window_surface_t(EGLDisplay dpy, EGLConfig config,
        EGLint surfType, ANativeWindow *window) :
        egl_surface_t(dpy, config, surfType), nativeWindow(window), buffer(NULL) {
...
egl_window_surface_t* egl_window_surface_t::create(EGLDisplay dpy,
        EGLConfig config, EGLint surfType, ANativeWindow *window) {
    egl_window_surface_t *wnd = new egl_window_surface_t(dpy, config, surfType,
            window);
    if (wnd && !wnd->init()) {
        delete wnd;
        wnd = NULL;
...
struct egl_pbuffer_surface_t: public egl_surface_t {
    ...
    uint32_t getRcColorBuffer() {
        return rcColorBuffer;
    }
...
egl_pbuffer_surface_t::egl_pbuffer_surface_t(EGLDisplay dpy, EGLConfig config,
        ...
        egl_surface_t(dpy, config, surfType), rcColorBuffer(0) {
    setWidth(w);
    setHeight(h);
}
...
static EGLClient_eglInterface s_eglIface = { getThreadInfo: getEGLThreadInfo,
...
__eglMustCastToProperFunctionPointerType eglGetProcAddress(
    ...
    return (__eglMustCastToProperFunctionPointerType ) ClientAPIExts::getProcAddress(
            procname);
...
EGLBoolean eglChooseConfig(EGLDisplay dpy, const EGLint *attrib_list,
    ...
    *num_config = rcEnc->rcChooseConfig(rcEnc, (EGLint*) attrib_list,
            attribs_size * sizeof(EGLint), (uint32_t*) tempConfigs,
            config_size);
    if (configs != NULL) {
        EGLint i = 0;
...
EGLSurface eglCreateWindowSurface(EGLDisplay dpy, EGLConfig config,
    ...
    EGLint surfaceType;
    if (s_display.getConfigAttrib(config, EGL_SURFACE_TYPE, &surfaceType)
            == EGL_FALSE)
        return EGL_FALSE;
    ...
    egl_surface_t *surface = egl_window_surface_t::create(&s_display, config,
            EGL_WINDOW_BIT, static_cast<ANativeWindow*>(win));
    if (!surface) {
...
EGLBoolean eglReleaseThread() {
    EGLThreadInfo *tInfo = getEGLThreadInfo();
    if (tInfo && tInfo->currentContext) {
        return eglMakeCurrent(&s_display, EGL_NO_CONTEXT, EGL_NO_SURFACE,
                EGL_NO_SURFACE);
    }
    return EGL_TRUE;
}
...
EGLContext eglCreateContext(EGLDisplay dpy, EGLConfig config,
    ...
    uint32_t rcContext = rcEnc->rcCreateContext(rcEnc, (uintptr_t) config,
            rcShareCtx, version);
    if (!rcContext) {
...
EGLBoolean eglMakeCurrent(EGLDisplay dpy, EGLSurface draw, EGLSurface read,
    ...
    if (context && (context->flags & EGLContext_t::IS_CURRENT)
            && (context != tInfo->currentContext)) {
        //context is current to another thread
        setErrorReturn(EGL_BAD_ACCESS, EGL_FALSE);
    }

    DEFINE_AND_VALIDATE_HOST_CONNECTION(EGL_FALSE);
    if (rcEnc->rcMakeCurrent(rcEnc, ctxHandle, drawHandle, readHandle)
            == EGL_FALSE) {
...
EGLImageKHR eglCreateImageKHR(EGLDisplay dpy, EGLContext ctx, EGLenum target,
    ...
    if (target == EGL_NATIVE_BUFFER_ANDROID) {
        ...
        uint32_t img = rcEnc->rcCreateClientImage(rcEnc, ctxHandle, target,
                texture);
        EGLImage_t *image = new EGLImage_t();
        image->dpy = dpy;
        image->target = target;
