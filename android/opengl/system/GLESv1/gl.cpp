...
//GL extensions
void glEGLImageTargetTexture2DOES(void *self, GLenum target,
        GLeglImageOES img) {
    (void) self;
    ...
    if (image->target == EGL_NATIVE_BUFFER_ANDROID) {
        ...
        GET_CONTEXT;
        ctx->override2DTextureTarget(target);
        GLeglImageOES hostImage =
                reinterpret_cast<GLeglImageOES>((intptr_t) image->host_egl_image);
        ctx->m_glEGLImageTargetTexture2DOES_enc(self, target, hostImage);
        ctx->restore2DTextureTarget();
    }
}

void glEGLImageTargetRenderbufferStorageOES(void *self, GLenum target,
        GLeglImageOES image) {
    (void) self;
    (void) target;

    DBG("glEGLImageTargetRenderbufferStorageOES v1 target=%#x image=%p", target,
            image);
    //TODO: check error - we don't have a way to set gl error
...
void init() {
    ...
    ctx->glEGLImageTargetRenderbufferStorageOES =
