...
HostConnection::HostConnection() :
        m_stream(NULL), m_glEnc(NULL), m_gl2Enc(NULL), m_rcEnc(NULL), m_checksumHelper() {
}
...
HostConnection* HostConnection::get() {
    ...
    if (tinfo->hostConn == NULL) {
        ...
        if (useQemuPipe) {
            ...
            if (!stream) {
                ALOGE(
                        "Failed to create QemuPipeStream for host connection!!!\n");
                delete con;
                return NULL;
            }
            if (stream->connect() < 0) {
        ...
        unsigned int *pClientFlags = (unsigned int*) con->m_stream->allocBuffer(
                sizeof(unsigned int));
        *pClientFlags = 0;
        con->m_stream->commitBuffer(sizeof(unsigned int));

        ALOGD(
                "HostConnection::get() New Host Connection established %p, tid %d\n",
                con, gettid());
        tinfo->hostConn = con;
    }
...
GLEncoder* HostConnection::glEncoder() {
    if (!m_glEnc) {
        ...
        DBG("HostConnection::glEncoder new encoder %p, tid %d", m_glEnc,
                gettid());
...
GL2Encoder* HostConnection::gl2Encoder() {
    if (!m_gl2Enc) {
        ...
        DBG("HostConnection::gl2Encoder new encoder %p, tid %d", m_gl2Enc,
                gettid());
...
void HostConnection::setChecksumHelper(renderControl_encoder_context_t *rcEnc) {
    ...
    if (extensionSize < 0) {
        ...
        extensionSize = rcEnc->rcGetGLString(rcEnc, GL_EXTENSIONS,
                glExtensions.get(), -extensionSize);
        if (extensionSize <= 0) {
            glExtensions.reset();
        }
    }
