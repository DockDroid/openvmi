...
static int map_buffer(cb_handle_t *cb, void **vaddr) {
    ...
    void *addr = mmap(0, cb->ashmemSize, PROT_READ | PROT_WRITE, MAP_SHARED,
            cb->fd, 0);
    if (addr == MAP_FAILED) {
        return -errno;
    }
    ...
    if (format == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED) {
        ...
        if (format == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED) {
            ALOGE("gralloc_alloc: Requested auto format selection, "
                    "but no known format for this usage: %d x %d, usage %x", w,
                    h, usage);
            return -EINVAL;
        }
        ...
        if (format == HAL_PIXEL_FORMAT_YCbCr_420_888) {
            ALOGE("gralloc_alloc: Requested YCbCr_420_888, but no known "
                    "specific format for this usage: %d x %d, usage %x", w, h,
                    usage);
        }
    }
...
static int fb_setUpdateRect(struct framebuffer_device_t *dev, int l, int t,
        int w, int h) {
    fb_device_t *fbdev = (fb_device_t*) dev;
...
static int gralloc_register_buffer(gralloc_module_t const *module,
    ...
    if (cb->ashmemSize > 0 && cb->mappedPid != getpid()) {
        ...
        int err = map_buffer(cb, &vaddr);
        if (err) {
            ERR("gralloc_register_buffer(%p): map failed: %s", cb,
                    strerror(-err));
            return -err;
        }
        cb->mappedPid = getpid();
...
static int gralloc_lock(gralloc_module_t const *module, buffer_handle_t handle,
    ...
    if ((hw_read || hw_write)
            || (!sw_read && !sw_write && !hw_cam_write && !hw_cam_read
                    && !hw_vid_enc_read) || (sw_read && !sw_read_allowed)
    ...
    //
    if (cb->canBePosted() || sw_read || sw_write || hw_cam_write || hw_cam_read
            || hw_vid_enc_read) {
    ...
    if (cb->hostHandle) {
        ...
        //
        int hostSyncStatus = rcEnc->rcColorBufferCacheFlush(rcEnc,
                cb->hostHandle, postCount, sw_read);
        if (hostSyncStatus < 0) {
        ...
        if (sw_read) {
            D("gralloc_lock read back color buffer %d %d\n", cb->width,
                    cb->height);
            rcEnc->rcReadColorBuffer(rcEnc, cb->hostHandle, 0, 0, cb->width,
                    cb->height, cb->glFormat, cb->glType, cpu_addr);
        }
    }
...
static int gralloc_unlock(gralloc_module_t const *module,
    ...
    if (cb->hostHandle) {
        ...
        if (cb->lockedWidth < cb->width || cb->lockedHeight < cb->height) {
            ...
            char *src = (char*) cpu_addr + cb->lockedTop * src_line_len
                    + cb->lockedLeft * bpp;
            char *dst = tmpBuf;
            ...
            rcEnc->rcUpdateColorBuffer(rcEnc, cb->hostHandle, cb->lockedLeft,
                    cb->lockedTop, cb->lockedWidth, cb->lockedHeight,
                    cb->glFormat, cb->glType, tmpBuf);

            delete[] tmpBuf;
        } else {
            rcEnc->rcUpdateColorBuffer(rcEnc, cb->hostHandle, 0, 0, cb->width,
                    cb->height, cb->glFormat, cb->glType, cpu_addr);
        }
    }
...
static int gralloc_lock_ycbcr(gralloc_module_t const *module,
    ...
    if ((!hw_cam_write && !sw_read) || (sw_read && !sw_read_allowed)) {
    ...
    case HAL_PIXEL_FORMAT_YCrCb_420_SP:
        ...
        break;
    default:
        ALOGE("gralloc_lock_ycbcr unexpected internal format %x", cb->format);
        return -EINVAL;
    }
    ...
    DD(
            ...
            usage, ycbcr->y, ycbcr->cb, ycbcr->cr, ycbcr->ystride,
            ycbcr->cstride, ycbcr->chroma_step);
...
//
static struct hw_module_methods_t gralloc_module_methods = { open
        : gralloc_device_open };

struct private_module_t HAL_MODULE_INFO_SYM = { base
        : { common: { tag: HARDWARE_MODULE_TAG, module_api_version
                : GRALLOC_MODULE_API_VERSION_0_2, hal_api_version: 0, id
                : GRALLOC_HARDWARE_MODULE_ID, name
                : "Graphics Memory Allocator Module", author
                : "The Android Open Source Project", methods
                : &gralloc_module_methods, dso: NULL, reserved: { 0, } },
                registerBuffer: gralloc_register_buffer, unregisterBuffer
                        : gralloc_unregister_buffer, lock: gralloc_lock, unlock
                        : gralloc_unlock, perform: NULL, lock_ycbcr
                        : gralloc_lock_ycbcr, } };
...
#else
    module = dlopen("/system/lib/hw/gralloc.default.so",
            RTLD_LAZY | RTLD_LOCAL);
#endif
    if (module != NULL) {
        sFallback = reinterpret_cast<gralloc_module_t*>(dlsym(module,
