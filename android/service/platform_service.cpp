...
namespace android {
PlatformService::PlatformService(const std::shared_ptr<openvmi::PlatformApiStub> &platform_api_stub) :
...
openvmi::PlatformApiStub::WindowStateUpdate::Window PlatformService::unpack_window_state(const Parcel &data) {
    ...
    return openvmi::PlatformApiStub::WindowStateUpdate::Window{
...
status_t PlatformService::update_window_state(const Parcel &data) {
    openvmi::PlatformApiStub::WindowStateUpdate state;
...
status_t PlatformService::update_application_list(const Parcel &data) {
    openvmi::PlatformApiStub::ApplicationListUpdate update;
    const auto num_packages = data.readInt32();
    for (auto n = 0; n < num_packages; n++) {
        String8 name(data.readString16());
        String8 package_name(data.readString16());

        auto p = openvmi::PlatformApiStub::ApplicationListUpdate::Application{
                name.string(),
                package_name.string(),
        };
        ...
        if (p.icon.size() > 0) {
          openvmi::PlatformApiStub::ApplicationListUpdate with_icon_update;
...
status_t PlatformService::set_clipboard_data(const Parcel &data) {
    ...
    const auto clip_data = openvmi::PlatformApiStub::ClipboardData{text.string()};
